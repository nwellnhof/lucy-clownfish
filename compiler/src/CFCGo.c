...
S_gen_autogen_go(CFCGo *self, CFCParcel *parcel) {
    ...
    char *class_vars  = CFCUtil_strdup("");
    char *class_inits = CFCUtil_strdup("");
    char *class_regs  = CFCUtil_strdup("");
    ...
    for (int i = 0; registry[i] != NULL; i++) {
        ...
        char *class_var = CFCGoClass_gen_class_var(class_binding);
        class_vars = CFCUtil_cat(class_vars, class_var, NULL);
        FREEMEM(class_var);

        char *class_init = CFCGoClass_gen_class_init(class_binding);
        class_inits = CFCUtil_cat(class_inits, class_init, NULL);
        FREEMEM(class_init);

        char *class_reg = CFCGoClass_gen_class_reg(class_binding);
        class_regs = CFCUtil_cat(class_regs, class_reg, NULL);
        FREEMEM(class_reg);
    }

    if (strlen(class_regs)) {
        char pattern[] =
            "\t%sRegisterClasses([]%sObjCLASS{\n"
            "%s"
            "\t})\n"
            ;
        char *temp = CFCUtil_sprintf(pattern, clownfish_dot, clownfish_dot,
                                     class_regs, clownfish_dot);
        FREEMEM(class_regs);
        class_regs = temp;
    }
    ...
    char pattern[] =
        ...
        ;
    char *content
        = CFCUtil_sprintf(pattern, type_decs, class_vars, class_inits,
                          class_regs, boilerplate, ctors, meth_defs);
